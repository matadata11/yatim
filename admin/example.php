...
if (isset($_GET['pg'])) {
	...
	if ($pg == 'level') {
		if ($input['action'] === 'edit') {
			mysqli_query($koneksi, "UPDATE pengawas SET nama='" . $input['nama'] . "' WHERE id_pengawas='" . $input['id'] . "'");
		} else if ($input['action'] === 'delete') {
			mysqli_query($koneksi, "delete from  pengawas WHERE id_pengawas='" . $input['id'] . "'");
		}
	}
